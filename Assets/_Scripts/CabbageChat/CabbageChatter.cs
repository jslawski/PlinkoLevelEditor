...
public GameObject chatCanvasObject;
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.CompareTag("Bounce") && this.shootCooldownActive)
        {
            ...
            this.cabbageRigidbody.AddForce(2.0f * new Vector3(this.cabbageRigidbody.linearVelocity.x * (float)reflectNormal.x, this.cabbageRigidbody.linearVelocity.y * (float)reflectNormal.y, this.cabbageRigidbody.linearVelocity.z * (float)reflectNormal.z));
        }
        ...
        if (collision.gameObject.CompareTag("Bumper"))
        {
            ...
            if (this.cabbageRigidbody.linearVelocity.magnitude > this.launchVelocity)
            {
                this.cabbageRigidbody.linearVelocity = this.cabbageRigidbody.linearVelocity.normalized * this.launchVelocity;
            }
    ...
    private IEnumerator ApplyArcForce()
    {
        ...
        float directionalArcForce = (this.cabbageRigidbody.linearVelocity.x > 0) ? -this.arcForce : this.arcForce;

        while (true)
        {
            this.cabbageRigidbody.AddForce(Quaternion.Euler(0, 0, 90) * this.cabbageRigidbody.linearVelocity * directionalArcForce);
            yield return null;
        }
    }
